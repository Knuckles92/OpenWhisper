...
- **Intelligence**: Pi SDK is the main supported agent core (TypeScript, bundled managed Node sidecar), running OpenRouter models; adapter boundary so direct-OpenRouter/OpenCode can swap in; v1 agent authority = meeting-state only (validated structured state patches + questions; no shell/fs/external actions).
...
- `meeting/` — standalone-extractable package, **no Qt imports**:
  ...
  - `capture/` — `devices.py` (find `"[Loopback]"` WASAPI input devices matching default render device), `sd_stream.py` (primary, sounddevice ≥ 0.5.0), `soundcard_stream.py` (fallback), `spool.py` (VAD-aligned chunk cuts: ≥400ms quiet after 20s, hard cut 32s; atomic WAV writes; gap-fill zeros when loopback goes silent >120ms).
...
- Settings: `SettingsKey` + `resolve_*()` validators + `config.py` defaults — `MEETING_WHISPER_MODEL`, `MEETING_LLM_PROVIDER/MODEL` (default from cleanup config), `MEETING_AGENT_CORE` ("pi"), `MEETING_CLOUD_CONSENT_GIVEN`, `MEETING_CLOUD_LAST_ENABLED`, `MEETING_SERVER_BIND` ("localhost"|"lan"), `MEETING_SERVER_PORT` (0 = ephemeral); `'meeting_toggle': ''` in `DEFAULT_HOTKEYS`.
