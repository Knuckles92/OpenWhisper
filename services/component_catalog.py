...
for _id, _name, _source, _license, _description in (
    ...
    _CATALOG[_id] = ComponentDetails(
        ...
        requires=("Windows x64 or Apple Silicon macOS (CPU)" if _id == "asr-nvidia-cpu" else "Windows x64" + (" and a CUDA-compatible NVIDIA GPU" if _id.endswith("cuda") else "")),
        payload="Verified native libraries; portable Python on Windows" if _id == "asr-nvidia-cpu" else "Portable Python 3.12 and verified runtime binaries",
