...
# The component payload, pinned at release time. GPU entries point at PyPI
# wheels; the meeting agent pins official Node from nodejs.org plus a sidecar
# zip attached to the same GitHub Release as the app installer. A published
# artifact's URL and contents are immutable, and the SHA-256 below is verified
# before anything is extracted. Regenerate with
# ``python scripts/build_component.py gpu-accel`` or
...
# Official Node 22 LTS zip (nodejs.org, immutable) plus the sidecar bundle zip
# attached to the v2.3.0 GitHub Release. Regenerated by
# ``python scripts/build_component.py meeting-agent``.
_BUILTIN_MEETING_AGENT_ARCHIVES: Final[Tuple[dict, ...]] = (
    ...
    },
    {
        ...
        "url": (
            "https://github.com/Knuckles92/OpenWhisper/releases/download/v2.3.0/"
