...
class TestSensitivityGateHonesty(TypeSafeMessagingTestCase):
    """A privacy switch must not imply protection it is not providing."""

    def _gate_warning(self, dialog):
        label = dialog.cleanup_sensitivity_gate_status
        return "" if label.isHidden() else label.text()

    def test_unkeyed_gate_says_it_is_screening_nothing(self):
        dialog = self._open(**{
            SettingsKey.TRANSCRIPT_CLEANUP_ENABLED: True,
            SettingsKey.TYPESAFE_ENABLED: True,
            SettingsKey.TYPESAFE_CLEANUP_SENSITIVITY_GATE: True,
        })
        warning = self._gate_warning(dialog)
        self.assertIn("Not screening anything", warning)
        self.assertIn("no TypeSafe API key is saved", warning)

    def test_master_switch_off_is_reported_as_the_reason(self):
        self.store.set(TYPESAFE_CREDENTIAL_ENV, TYPESAFE_KEY)
        dialog = self._open(**{
            SettingsKey.TRANSCRIPT_CLEANUP_ENABLED: True,
        ...
        self.assertIn("fast judgments is off", self._gate_warning(dialog))

    def test_no_warning_when_the_gate_can_actually_run(self):
        self.store.set(TYPESAFE_CREDENTIAL_ENV, TYPESAFE_KEY)
        dialog = self._open(**{
            SettingsKey.TRANSCRIPT_CLEANUP_ENABLED: True,
            SettingsKey.TYPESAFE_ENABLED: True,
        ...
        self.assertEqual(self._gate_warning(dialog), "")

    def test_no_warning_when_the_gate_is_off(self):
        dialog = self._open(**{
            SettingsKey.TRANSCRIPT_CLEANUP_ENABLED: True,
            SettingsKey.TYPESAFE_ENABLED: True,
            SettingsKey.TYPESAFE_CLEANUP_SENSITIVITY_GATE: False,
        })
        self.assertEqual(self._gate_warning(dialog), "")

    def test_no_warning_when_cleanup_itself_is_off(self):
        # Nothing is leaving the machine, so "sent to the cloud unchecked"
        # would be false.
        dialog = self._open(**{
    ...
    def test_no_warning_for_a_local_cleanup_endpoint(self):
        # Local endpoints are never screened with or without a key, which the
        # tile description already states.
        dialog = self._open(**{
            SettingsKey.TRANSCRIPT_CLEANUP_ENABLED: True,
            SettingsKey.TYPESAFE_ENABLED: True,
            SettingsKey.TYPESAFE_CLEANUP_SENSITIVITY_GATE: True,
        })
        self.assertNotEqual(self._gate_warning(dialog), "")
        with patch.object(
            settings_dialog_module, "resolve_transcript_cleanup_provider",
            return_value="ollama",
        ):
            dialog._render_typesafe_key_state()
        self.assertEqual(self._gate_warning(dialog), "")

    def test_an_unknown_cleanup_endpoint_is_treated_as_remote(self):
        dialog = self._open(**{
            SettingsKey.TRANSCRIPT_CLEANUP_ENABLED: True,
            SettingsKey.TYPESAFE_ENABLED: True,
            SettingsKey.TYPESAFE_CLEANUP_SENSITIVITY_GATE: True,
        ...
        with patch.object(
            settings_dialog_module, "get_text_llm_profile",
            side_effect=RuntimeError("catalog unreadable"),
        ):
            dialog._render_typesafe_key_state()
        self.assertIn("Not screening anything", self._gate_warning(dialog))
