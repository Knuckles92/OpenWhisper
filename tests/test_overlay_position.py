...
class TestClampRectToAvailable:
    @pytest.fixture(autouse=True)
    def _setup(self):
        self.screen = QRect(0, 0, 1920, 1080)

    def test_keeps_in_bounds_position(self):
        assert clamp_rect_to_available(100, 200, 300, 80, self.screen) == (100, 200)
    ...
    def test_clamps_left_and_top_overflow(self):
        assert clamp_rect_to_available(-50, -20, 300, 80, self.screen) == (8, 8)
...
class TestPreferredOverlayPosition:
    ...
    def test_growing_height_near_bottom_moves_up(self):
        ...
        assert short == (210, 1000 - 80 - 10)
        assert tall[1] < short[1]
        assert tall[1] >= 8
        assert tall[1] + 400 <= 1080 - 8


class TestMaxHeightForAnchor:
    @pytest.fixture(autouse=True)
    def _setup(self):
        self.screen = QRect(0, 0, 1920, 1080)

    def test_respects_configured_max_when_roomy(self):
        assert max_height_for_anchor(QPoint(100, 100), self.screen, 400) == 400
    ...
    def test_limits_by_space_on_short_display(self):
        ...
        assert height == 200 - 10 - 8
        assert height < 400

    def test_never_returns_below_one(self):
        tiny = QRect(0, 0, 100, 20)
        assert max_height_for_anchor(QPoint(10, 10), tiny, 400) >= 1
