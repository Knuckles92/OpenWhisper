"""Shared sidecar supervision and meeting-tool authority over stdio RPC.
...
class SidecarAgent:
    ...
    def initialize(self, cfg: AgentConfig, tools: AgentToolHost) -> None:
        ...
        if not api_key:
            self._fatal = True
            raise RuntimeError(
                f"No {cfg.provider} API key found; meeting sidecar agent offline"
            )
        ...
        logger.info(
            "Meeting sidecar initialized (provider=%s model=%s pi_version=%s)",
    ...
    def _cancel_request(self, request_id: str) -> bool:
        ...
        with self._tool_authority_lock:
            with self._lock:
                self._revoked_requests.add(request_id)
        try:
            result = self._rpc(
                "cancel",
    ...
    def _run_checkpoint(self, payload: CheckpointPayload,
        ...
        try:
            ...
            if isinstance(exc, TimeoutError):
                ...
                return AgentResult(ok=False, error=str(exc), op_results=list(self._checkpoint_op_results.get(payload.request_id, [])))
            logger.warning(
    ...
    def _endpoint_fields(self) -> Dict[str, Any]:
        ...
        if not spec.tools:
            raise RuntimeError("This model cannot use meeting tools with this meeting engine.")
        return {
    ...
    def _spawn_and_handshake(self, api_key: str) -> None:
        ...
        logger.info("Spawning meeting sidecar: %s", cmd)
        try:
            ...
            self._fatal = True
            raise RuntimeError(f"failed to spawn meeting sidecar: {exc}") from exc
    ...
    def _on_stdout_closed(self, generation: int, proc: subprocess.Popen) -> None:
        ...
        if not self._shut_down and not self._fatal:
            logger.warning("Meeting sidecar process exited unexpectedly")
    ...
    def _handle_tool_request(self, msg: Dict[str, Any],
        ...
        # already admitted operation, then no subsequent operation may start.
        with self._tool_authority_lock:
            self._handle_tool_request_authorized(msg, generation)

    def _handle_tool_request_authorized(self, msg: Dict[str, Any],
                             generation: Optional[int] = None) -> None:
    ...
    def _recover_locked(self, reason: str) -> bool:
        ...
        if len(self._restart_times) >= _MAX_RESTARTS:
            ...
            logger.error(
                "Meeting sidecar restart budget exhausted (%d/%d in %.0fs); "
        ...
        logger.warning(
            "Restarting meeting sidecar in %.0fs (attempt %d/%d): %s",
        ...
        try:
            ...
            logger.exception("Meeting sidecar restart failed")
            self._restart_times.append(time.monotonic())
            if len(self._restart_times) >= _MAX_RESTARTS:
                self._fatal = True
                self._initialized = False
                logger.error(
                    "Meeting sidecar restart budget exhausted; intelligence offline"
                )
            return False
        ...
        self._ping_misses = 0
        self._initialized = True
        logger.info("Meeting sidecar restarted successfully")
        return True
