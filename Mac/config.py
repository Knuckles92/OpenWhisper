...
class AppConfig:
    ...
    # Whisper model - "auto" selects based on hardware. On macOS there is no
    # CUDA, so "auto" resolves to CPU (base model, int8 compute).
    DEFAULT_WHISPER_MODEL: str = "auto"

    # Faster-whisper settings. CUDA is unavailable on macOS (faster-whisper has
    # no MPS/Metal backend), so "auto" runs on CPU here.
    FASTER_WHISPER_DEVICE: str = "auto"  # "auto", "cpu" (cuda N/A on macOS)
    FASTER_WHISPER_COMPUTE_TYPE: str = "auto"  # "auto", "int8", "float32" (float16 needs GPU)
