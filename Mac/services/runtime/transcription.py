...
_keyboard_controller: "_KeyboardController | None" = None


def _send_cmd_v() -> None:
    """Simulate Cmd+V (paste) on macOS via pynput.

    Requires Accessibility permission for the host process to post synthetic
    key events.
    """
    global _keyboard_controller
    if _keyboard_controller is None:
        _keyboard_controller = _KeyboardController()
    with _keyboard_controller.pressed(_Key.cmd):
