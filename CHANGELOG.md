...
- **Keep saved recordings by folder size** — Settings → Recording → Keep recordings now offers **By folder size** alongside Keep all and a set number of recordings (renamed **By count**). Choose a limit in MB and the oldest Quick Record audio files are deleted once the recordings folder grows past it; the newest recording is always kept, even if it alone is larger than the limit. The tile shows how many recordings are saved and how much space they use. Transcription history text is unaffected.

### Changed
- **Settings, Model Manager, and Downloads are one window** — Settings now opens on an **Overview** of what is running: the dictation and meeting voice models, AI cleanup, meeting intelligence, hotkeys, which of those stay on this computer and which are sent to a cloud provider, downloaded models by backend, and installed components. Every card opens its page. The rail is grouped by feature (Dictation, Meeting Mode, Models & storage, App), so each model choice sits on the page for the feature it powers: the cleanup chat model is on **AI cleanup**, the meeting chat model and agent core are on **Intelligence**, and the dictation and meeting speech models are on **Voice model** and **Voice & speakers**. **Downloads** and **Runtime** are pages of the same window, and the Hugging Face download policy moved from Advanced to Downloads. The Downloads rail item shows live download progress, and the model profile docks beside the catalog when there is room and slides over it when there is not. The separate Model Manager window, its footer button, and the File menu entry are gone; File → Downloads… and every link that used to open Model Manager now open the matching Settings page.
...
### Fixed
- Meeting Mode opens correctly in packaged desktop builds without a console. The dashboard now uses the application logging configuration, avoiding the startup error "Unable to configure formatter 'default'".
...
- **Sensitive-dictation gate** for cloud cleanup (Settings → Dictation → AI cleanup). Dictation bound for a remote cleanup model is first sent to TypeSafe to check for credentials, identifiers, and personal details; flagged text is returned raw without reaching the cleanup model. Local endpoints are never screened. If the check is unavailable, cleanup proceeds.
- **Experimental end-of-meeting insight review** (Settings → Meeting Mode → After End, off by default). With TypeSafe sharing consent and meeting cloud intelligence enabled, the dashboard asks source-linked questions about uncertain action items, decisions, and risks. Answers update the linked insight, preserve protected user clarifications in reports and exports, and can be revised later from Past Meetings. Recording and saving do not wait for review; stale questions cannot overwrite newer edits.
