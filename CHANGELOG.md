...
- **Model Manager is organized by product mode** - On-demand and Meeting Mode each show the Voice and Text models they actually use. A shared Library tab owns Whisper downloads, optional components, and the device/quantization runtime both modes inherit. Meeting spoken language, agent core, and speaker identification moved out of Settings → Meeting so the manager is the single owner of every meeting model choice. Settings → Cleanup and Settings → Meeting keep behavior, prompts, and dashboard controls, with a read-only recap and a link back to the manager
